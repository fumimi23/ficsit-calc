...
// issue #6 で数値表現を分数(Fraction)に変更し、数値検証を近似(toBeCloseTo)から厳密等値に強化した。
...
describe("計算コア(issue #1)", () => {
	it("鉄板 30 個/分 を指定したとき、構築機 1.5 台・製錬炉 1.5 台(鉄インゴット 45/分)・鉄鉱石 45 個/分・合計電力 12MW(各 4MW × 3.0 台)が算出される", () => {
		...
		expect(plate?.machineCount).toEqual(frac(3, 2));
		expect(ingot?.building).toBe("smelter");
		expect(ingot?.machineCount).toEqual(frac(3, 2));

		const ingotNode = plan.root.inputs.find((n) => n.item === "iron-ingot");
		expect(ingotNode?.ratePerMinute).toEqual(frac(45));

		expect(plan.rawMaterials).toEqual([
			{ item: "iron-ore", ratePerMinute: frac(45) },
		]);

		expect(plan.totalPowerMW).toEqual(frac(12));
	});
	...
	it("同一中間素材が複数の枝から要求されるとき、素材・機械・電力の合計に重複なく合算される", () => {
		...
		expect(ingotEntries[0]?.machineCount).toEqual(frac(2));

		// 原料の合算も 1 エントリ: 鉄鉱石 60/分
		expect(plan.rawMaterials).toEqual([
			{ item: "iron-ore", ratePerMinute: frac(60) },
		]);

		// 合計電力: 組立機 1.0×15 + 構築機(鉄板 1.5 + ネジ 1.5 + ロッド 1.0)×4 + 製錬炉 2.0×4 = 39MW
		expect(plan.totalPowerMW).toEqual(frac(39));
	});
	...
	it("機械台数は小数のまま保持する(丸め・クロック提案は表示側の関心事)", () => {
		...
		const plate = plan.machines.find((m) => m.recipeId === "iron-plate");
		const ingot = plan.machines.find((m) => m.recipeId === "iron-ingot");
		expect(plate?.machineCount).toEqual(frac(1, 2));
		expect(ingot?.machineCount).toEqual(frac(1, 2));
		expect(plan.root.production?.machineCount).toEqual(frac(1, 2));
	});
});
